...
KDoc is not limited to ports: **every public type and non-obvious member** carries KDoc that explains WHY (not what) and cites related types with `[Type]` links.

### Named arguments — mandatory for multi-arg constructors

Call sites with more than two arguments always use named arguments:
...
Positional arguments on multi-param constructors are a review blocker — they silently survive parameter reordering.
...
- **ktlint via Spotless** with `spotless:check` bound at the `validate` phase — formatting is a build gate, not a suggestion. `spotless:apply` fixes locally; the build fails on drift.
- **Imports explicit and ordered, no wildcards** (ktlint-enforced).
...
- **`Xjsr305=strict` compiler flag** — strict JSR-305 nullability analysis for Java interop.

---

## Sonar Analyzer Gaps

**SonarKotlin S1481 ("unused local") false-positives on locals used only in indexed assignment** — the analyzer does not count `map[key] = value` as a read of the local. When S1481 fires on such a local, do NOT delete it: verify the value really is read by the assignment, then silence the finding by inlining — into the assignment RHS, or into a `.let { }` lambda parameter (which S1481 tracks).

```kotlin
val v = computeValue()
map[k] = v            // S1481 flags v as unused — false positive

map[k] = computeValue()                        // fix: inline into RHS
computeValue().let { v -> map[k] = v }         // fix: .let lambda parameter
```
