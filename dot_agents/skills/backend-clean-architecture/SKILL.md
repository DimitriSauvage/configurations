...
│   ├── model/          ← use-case outputs: *Result / *Item / *Detail (application models, framework-free)
│   ├── port/
...
└── adapter/
    ...
    │       ├── doc/         ← composed OpenAPI response-doc annotations
    │       └── errormanager/← ApplicationException carrier + ApplicationErrorHandler + ErrorResponse
    └── outbound/
        └── persistence/
...
**Rules:**
- Domain knows NOTHING about Spring, JPA, MongoDB, HTTP — and no Jackson or `jakarta.*` either (pure Kotlin only)
- Application knows NOTHING about controllers, JPA entities, HTTP status — no HTTP types (`jakarta.servlet`, `org.springframework.http`, `org.springframework.web`), no JPA (`jakarta.persistence`, `org.springframework.data`)
- Neither domain nor application may depend on `..config..` — config wires everything, nothing depends on it
- No cyclic dependencies between top-level packages (`com.<base>.(*)..`) — ArchUnit `slices()` must be free of cycles
...
- Controllers inject **use-case ports**, never `..application.service..` implementations (ArchUnit rule)
...
| Domain error | `<Context>DomainError` sealed subclass per context | `JuryDomainError` (members like `JuryNameBlank`) |
| Domain repository port | `Port` or `Repository` | `JuryRepositoryPort`, `RecipeRepositoryPort` |
...
## Inbound Adapter Boundary — No Domain Types in DTOs

Inbound adapters (DTOs, controllers) must never reference `domain.model` types — enforced by ArchUnit (`adaptersInDoNotAccessDomain`), with mappers under `..inbound..mapper..` as the only exception. For enum vocabularies at the boundary:

- DTO fields are `String` + `@field:Pattern(regexp = "A|B|C")` + `@field:Schema(allowableValues = [...])` — Bean Validation rejects unknown values with 400.
- The application service parses the string to the domain enum (returning `UseCaseError.ValidationFailed` on unknown values) — commands may carry strings from the inbound boundary; domain objects are built inside the application/domain layer.
