...
**Source of errors — `ApiError`**: the fetch layer throws `ApiError extends Error` with `readonly status` on any non-200 (unwraps the Orval status union: `if (response.status !== 200) throw new ApiError(response.status, response.data)`). Its message is extracted from the server envelope via `unknown`-narrowing, falling back to `"Request failed with status N"`. The classifier trusts `err.status` — no re-parsing.
...
Rendering is a **v-if state machine in fixed order**: empty-input → pending → error → no-results → results. When the view is gated on user input (e.g. a search box), the **empty-input** guard renders first and NO query fires (`enabled` gate must also prevent the fetch).

| State | Condition | Render |
|---|---|---|
| Empty input | gated view, user input empty | Static hint, no fetch |
...
Optional 5th: `isFetching && !isPending` → subtle indicator + `aria-busy="true"` (background refetch). With `keepPreviousData` the list stays visible and **dims** instead of swapping to a skeleton.
