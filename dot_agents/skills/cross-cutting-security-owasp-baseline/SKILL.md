...
- Rule (multi-tenant isolation): Every tenant-scoped query filters by the tenant UNCONDITIONALLY — fail-closed. The tenant ID comes from the authenticated caller context (JWT claim / `CallerContext`), never from a query parameter or request body. A missing tenant context yields an empty or not-found result, never an unfiltered query.
- Check: Does every list/read/write path in a multi-tenant service apply the tenant filter even when no caller-supplied filter is present? Is the tenant ID sourced only from the authenticated context?

*(Multi-tenant rules from talent-api PR40 session, 2026-09-10.)*
...
- Rule (log injection): Strip control characters from user-controlled values BEFORE writing them to logs — e.g. a `sanitizeForLog(value)` helper replacing `\p{Cntrl}` with `_`. Control characters can forge log lines (A09-adjacent).
- Check: Is every log sink embedding user-controlled input routed through the sanitizer? (JSON response bodies are exempt only because Jackson encodes them.)
...
- Never write user-controlled input to a log line without stripping control characters (`\p{Cntrl}` → `_`, e.g. `sanitizeForLog`) — log-injection guard.
- Never rely on client-only authorization — enforce server-side.
- Never issue a tenant-scoped query without the tenant filter — the filter is unconditional (fail-closed), and the tenant ID comes only from the authenticated context, never client input.
