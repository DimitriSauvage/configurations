...
Route-scoped components in `pages/<route>/components/` depend on the pages scan excluding colocated components — `nuxt.config.ts` must carry `pages: { pattern: ['**/*.vue', '!**/components/**'] }` (cross-link `nuxt4-spa-conventions`). Without that pattern, colocated components would be scanned as routes.
...
The core authoring order is FIXED: **Props → Composables → State → Computed → Head → Watchers → Handlers**. Use `useHead()` in the Head region. Never create empty regions. Each region label is unique per file.

### Component Authoring Rules (app tier)

- **No `withDefaults`. No `defineEmits` in app-tier components** (Nuxt app code; the DS library is exempt). Children mutate shared state via composables or receive callback props — no custom events cross component boundaries. Inline object prop type allowed only for one-off props (e.g. `error.vue`: `defineProps<{ error: NuxtError }>()`).
- Event handlers are `function onXxx(...)` forwarding to composables/PrimeVue, **typed with the PrimeVue event payload shape** — e.g. `function onPageChange(event: { page: number }): void`, `function onSelect(event: { value: string }): void`.
- Pure display logic extracted to `<name>.utils.ts` pure functions (`formatMatchStatus(item)`), unit-tested separately.
- Rendering is a **v-if state machine in fixed order**: empty-input → pending (Skeleton, `aria-busy`) → error (`role="alert"` + retry Button) → no-results → results.
- Floating promises are `void`-prefixed: `void refetch()`, `void router.push("/")`.
- `useId()` for label/control pairing; `:for` and `:input-id` derive from it.
- Order-sensitive declarations get a rationale comment (e.g. a ref declared before the query wiring because vue-query evaluates the filters computed eagerly — TDZ).
- **Watch the list length, not the ref, when the composable mutates via `push()`**: a shallow ref watch misses in-place mutation. `watch([() => items.value.length, readyToCookOnly], ...)` — real bug class; lock with a regression test.
