...
## CI Pipeline (every push / PR — authoritative)

Backend and frontend run independently:

```sh
# backend — unit + Testcontainers integration tests (runner needs Docker)
./mvnw verify

# frontend — gates in this order, stop on first red
npm ci
npm run test
npm run lint
nuxt typecheck
...
- `nuxt typecheck` runs `vue-tsc --noEmit` through Nuxt — it is the typecheck gate, not an optional extra.
- Generated API client is regenerated before every consumer (`build`/`test` run `gen:api` first) — CI never builds against stale DTOs.
