...
## PrimeVue Theme Bridging via `@theme` — How `bg-surface-*`/`text-primary` Exist

Tailwind color utilities that follow the active PrimeVue theme exist because the app stylesheet bridges PrimeVue runtime variables (`--p-*`, injected by the Aura preset) into Tailwind tokens in `app/assets/css/main.css`:
...
@theme {
  ...
  --color-primary-active: var(--p-primary-active-color);
...
- `--color-<name>: var(--p-<variable>)` generates `bg-<name>`, `text-<name>`, `border-<name>` utilities resolving to the LIVE PrimeVue theme — the app never redeclares theme colors as raw hex.
- Bridging is the app-level equivalent of the DS semantic-token rule: a missing bridge shows up as the banned `text-[var(--p-*)]` arbitrary value. Bridge once, consume utilities everywhere.
- Reference prototype is light-only: the bridge carries no `dark:` overrides. DS-driven apps keep the dark-mode pairing rules above.
