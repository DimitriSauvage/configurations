...
### Paginated Lists — keepPreviousData Dimming

Paginated queries use `placeholderData: keepPreviousData`. During a page change the UI renders the PREVIOUS page dimmed (`opacity-*` + `aria-busy="true"`) instead of swapping in a skeleton.

- Why: pagination controls stay stable (no focus/scroll jump) and the user keeps their mental model of the list.
- Skeleton is only for the FIRST load — no previous data to keep.
...
- `loading="lazy"` + `decoding="async"` for below-fold images (decode off the main thread).
