...
- Domain value objects shared by 2+ bounded contexts live in the **shared kernel** at `common.domain.model.<concept>`; single-context VOs stay in that context's `domain/model/`. Root-level packages are invisible to the onion/layered rules but a `.domain.` segment still triggers the `..domain..` framework-purity rule. See the `backend-kotlin-best-practices` skill for the VO construction contract.
