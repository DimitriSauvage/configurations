...
### CRITICAL GUARDRAIL 4: Deterministic Rendering State Machine

- Mandatory: Async-driven regions render as a `v-if` state machine in FIXED order: empty-input → pending (Skeleton + `aria-busy`) → error (`role="alert"` + retry Button) → no-results → results.
- Constraint: The pending arm mounts a Skeleton and sets `aria-busy="true"` on its container; the error arm MUST expose `role="alert"` and a retry affordance — never a bare message.
- Prohibition: Never merge or reorder arms (e.g. rendering "no results" while pending). Each state is its own `v-if` arm with no fallthrough.

### CRITICAL GUARDRAIL 5: SFC Block Order and Style Ban

- Mandatory: `<script setup lang="ts">` first, `<template>` second.
- Prohibition: NO `<style>` block of ANY kind (scoped, module, plain) in the SFC — Tailwind utilities only.
...
- [ ] An async region deviates from the fixed-order `v-if` state machine (empty-input → pending Skeleton + `aria-busy` → error `role="alert"` + retry → no-results → results) or merges/falls through states.
- [ ] A `<style>` block (scoped, module, plain) exists in the SFC, or `<script setup lang="ts">` is not the first block.
- [ ] Below-fold `<img>`/`NuxtImg` elements are missing `loading="lazy"` and `decoding="async"`.
- [ ] Derived template values are computed inline instead of routed through a `computed` ref.
