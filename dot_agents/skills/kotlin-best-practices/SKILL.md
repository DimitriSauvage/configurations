...
Commands, query results, embedded models — all properties are domain objects. DTOs (adapters) are the exception and may hold primitives:

```kotlin
// Commands, application models, embedded models — VOs, never raw primitives (see the value-object contract below)
data class CreateJuryCommand(
    val companyId: CompanyId,
    val name: JuryName,
    val description: JuryDescription?,
)
...
data class AuditInfo(
    ...
    fun updatedNow(now: UpdatedAt) = copy(updatedAt = now)
}

// Adapters only (HTTP DTOs, persistence documents) may carry raw primitives
data class CreateJuryRequestDto(val name: String, val description: String?)
```
...
- **No raw primitives on domain/application models.** Every property of a type in `domain/model` or `application/model` (entities, value objects, commands, response models) is itself a domain object. Raw `String`, `Int`, `Long`, `Double`, `Boolean`, `Instant`, `LocalDate`, `UUID` are prohibited there — wrap them (`CreatedAt(Instant)`, `MessageContent(String)`, `FirstName(String)`, …). Primitives are allowed only in adapters (HTTP DTOs, persistence documents) and at the DTO→command / mapper seams.
- No redundant context prefix on shared concepts (`cv`, `profile`, `candidate`) — keep a prefix only if it is the source spec's field name or the concept is genuinely context-local (`Application*`).

### Value-Object Construction Contract

Every value object, without exception:

1. **Preprocess** the raw input first — trim, collapse whitespace, case-fold, strip separators.
2. **Validate** the preprocessed value; return `Result<V, DomainError>` — never throw.
3. **Format / normalize** to the canonical stored form (case, separators, precision).
4. **One file per value object.**
5. **Private constructor** + companion `of(raw: …): Result<V, DomainError>` as the only construction path for raw input.
6. **KDoc** states the preprocessing, the invariant, and the canonical format.

Even a no-op VO (no invariant, no normalization) gets its own file + documented `of(…)`, so a future rule has a home without a signature-breaking change.

```kotlin
// domain/model/identity/FirstName.kt — one file per VO
@JvmInline value class FirstName private constructor(val value: String) {
    companion object {
        /** Preprocess: trim + collapse. Validate: non-blank, ≤ 100. Format: Title Case. */
        fun of(raw: String): Result<FirstName, DomainError> {
            val pre = raw.trim().replace(Regex("\\s+"), " ")
            if (pre.isBlank() || pre.length > 100) return Err(DomainError.InvalidFirstName)
            val formatted = pre.split(" ").joinToString(" ") { it.replaceFirstChar { c -> c.titlecase() } }
            return Ok(FirstName(formatted))
        }
    }
}
```

Intent examples: `FirstName` → Title Case; `LastName` → UPPER CASE; `Email` → trim, lowercase, then regex-validate. Enumerated concepts stay `enum class` with a lowercase `wire` value and `fromWire(…)` (see the `constants-discipline` skill).

### Shared Kernel

- Required by **2+ bounded contexts** → **shared kernel**, one definition, at `com.beetween.talentapi.common.domain.model.<concept>`.
- Required by a single context → that context's `domain/model/`.
- Never merge two look-alike VOs that differ in semantics or validation.
- Provenance wrappers (e.g. `Traced<T>`) are owned by the context that needs them; the kernel holds the plain VOs and never imports the wrapper.
- A `.domain.` segment makes ArchUnit's `..domain..` framework-purity rule apply; root-level packages escape onion/layered classification.

VO contents, canonical formats, and validations are decisions reflected here and, when project-specific, in a local ADR (see `documentation-and-adrs`).
...
- **No fully-qualified type names in code — import them.** `fun com.example.X.f()` → `import com.example.X` + `fun X.f()`. Exception: naming collision → `import com.a.X as Y` alias (fall back to the FQN only when an alias reads worse). KDoc `[fully.qualified.Type]` links and ArchUnit package strings are not code references and stay as-is.
