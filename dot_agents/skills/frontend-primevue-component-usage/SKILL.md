...
If a `--p-*` variable has no generated class, bridge it once via `@theme { --color-<name>: var(--p-<variable>) }` in the app stylesheet and consume as `bg-<name>`/`text-<name>` (see `style-tailwind` → PrimeVue Theme Bridging). Never reference `var(--p-*)` inline in templates.
