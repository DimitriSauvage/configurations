...
export type BtColumnSettingsProps = BaseComponentProps & {
  columns?: BtColumnSettingsColumnToggle[];
  titleText?: string;
};
```
...
|------|------|---------|----------|-------------|
| `columns` | `BtColumnSettingsColumnToggle[]` | `[]` | No | Array of column definitions. Each entry has `key` (unique ID), `label` (displayed text), and `visible` (boolean). |
...
|-------|---------|-------------|
| `update:columns` | `BtColumnSettingsColumnToggle[]` | Emitted on every checkbox toggle with the full updated array. Enables `v-model:columns` binding. |
