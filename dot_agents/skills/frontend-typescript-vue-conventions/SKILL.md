...
Exception: inline object type allowed for one-off props (e.g. `error.vue`: `defineProps<{ error: NuxtError }>()`).

**No `withDefaults`. No `defineEmits` in app-tier components** (Nuxt app code; the DS library is exempt). Children mutate shared state via composables or receive callback props — no custom events cross component boundaries.
...
PrimeVue component events: type handlers with the **payload shape**, named `onXxx`:
...
`void`-prefix every promise you don't await: `void refetch()`, `void router.push("/")`. Silently discarded promises hide rejections.

---

## Generated Types (`app/api/generated/`)

Orval output — **READ-ONLY**. Gitignored. Regenerated before every consumer — `gen:api` (Orval) chained into `build`/`test` scripts: `"build": "npm run gen:api && nuxt build"`.
...
- **Non-query state composables are module-level singletons**: state lives at module scope, the returned refs are wrapped in `readonly()`, and mutations go through exported action functions (`add`/`remove`/`clear`). Persist to localStorage defensively: try/catch on write with a why-comment, `unknown` + type-guard on load, namespaced storage key.
...
ESLint is flat config. Ignored: generated code (`app/api/generated`), `.nuxt`, `.output`. `vue/multi-word-component-names` is off **only** for `pages/` and `error.vue`, with a comment. Prettier handles formatting only (`printWidth: 100`) — lint never checks formatting.
