...
| Article | `<article>` (self-contained result/item card) |
| Ordered list | `<ol>` (ranked results, steps — order is meaning) |

Exactly one `<main id="main-content">`. Headers/nav/footer/aside use implicit roles.
...
- Tailwind form: `focus-visible:outline-2 focus-visible:outline-primary` (+ `outline-offset-2`) — width AND color together, never a bare `outline`
...
## Rendering States — Loading, Error, Images

| State | Requirement |
|---|---|
| Pending | Skeleton + `aria-busy="true"` on the loading container |
| Error | Container `role="alert"` + a retry Button — announce, don't just color |
| No-results | Distinct empty state — never silently render nothing |
| Below-fold images | `loading="lazy" decoding="async"` |

- Image `alt` = the meaningful subject (entity name), `alt=""` decorative.
- These are the rendered arms of the async `v-if` state machine — see `template-building-enforcer` / component-authoring rules.
